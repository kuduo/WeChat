...
namespace HD.WeChat.API.CommonAPIs
{
    /// <summary>
    /// 通用接口
    ...
    public partial class CommonApi
    {
        ...
        /// <returns></returns>
        public static WXUserInfoResult GetUserInfo(string accessToken, string openId)
        {
            var url = string.Format("http://api.weixin.qq.com/cgi-bin/user/info?access_token={0}&openid={1}",
                                    accessToken, openId);
            WXUserInfoResult result = Get.GetJson<WXUserInfoResult>(url);
            return result;
        }
